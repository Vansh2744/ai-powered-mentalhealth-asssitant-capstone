  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div ref={ref} tabIndex={-1}
        ...
        style={{ border: `1.5px solid ${isCritical ? "#ff444460" : "#ffaa0060"}`,
                 boxShadow: `0 0 60px ${isCritical ? "#ff444430" : "#ffaa0025"}` }}>

        {/* Top bar */}
        <div className="px-6 py-4 flex items-center gap-3"
          style={{ background: isCritical ? "linear-gradient(135deg,#7f1d1d,#450a0a)" : "linear-gradient(135deg,#78350f,#292524)" }}>
            ...
            {crisis.helplines.map((h, i) => (
              <div key={i} className="rounded-xl px-4 py-3 flex items-center justify-between"
                ...
                <div>
                  ...
                  <p className="text-white/40 text-[0.6rem] tracking-wide">{h.available}</p>
                </div>
                <a href={`tel:${h.number.replace(/\s/g, "")}`}
                  className="px-3 py-1.5 rounded-full text-[0.65rem] font-bold tracking-widest"
                  style={{ background: isCritical ? "#ff4444" : "#f59e0b", color: "#000" }}>
                  {h.number}
                </a>
              </div>
            ))}
